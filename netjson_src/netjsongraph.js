...
(function() {
    ...
    d3.netJsonGraph = function(url, opts) {
        ...
        processJson = function(graph) {
            ...
            node = groups.append("circle").attr("class", function(node) {
                ...
                return baseClass;
            }).attr("r", opts.circleRadius).on("click", opts.onClickNode).call(drag);

            var labels = groups.append('text').text(function(n) {
                return n.label || n.id
            }).attr('dx', opts.labelDx).attr('dy', opts.labelDy).attr('class', 'njg-tooltip');
            ...
            var attrs = ["protocol", "version", "revision", "metric", "router_id", "topology_id"],
                html = "";
